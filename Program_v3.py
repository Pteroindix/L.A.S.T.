...
class workDays():
	...
	def workDays(self):
		...
		# not able to exclude holidays and teacher training days yet...
...
def mainPage():
	...
	stuID = scanner()
	global STUDENT
	STUDENT = sqlconnection(stuID,DAYNUM) #connect to database
	#print(cardDetected)
...
def sqlconnection(ID_Number, Day_Number):
	try:
		cnx = mysql.connector.connect(user="root",password="31415926535",\
			database="L.A.S.T.")
...
def sentOutPage():
	...
	sendEmail("LATE STUDENT - " + STUDENT[0],\
	"Hello,\n\n" + STUDENT[0] + " from " + STUDENT[1] + " is late for school on " + date\
...
def sendEmail(title,messages,toAddr):
	fromAddr= r'1121134659@qq.com'  # sender's email address (using personal QQmail at the moment)
	password = r'mvuewzosgrqwfjjc'  # sender's email passwords
